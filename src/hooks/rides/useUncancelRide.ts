...
export function useUncancelRide() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      return apiClient<{ success: boolean }>(`/rides/${id}/uncancel`, {
        method: "POST",
        token,
      });
    },
    onMutate: async (id) => {
      ...
      // Optimistically mark as not cancelled
      if (previousRide) {
      ...
      queryClient.setQueriesData<RideList[]>({ queryKey: ["rides"] }, (old) =>
        old?.map((ride) =>
          ride.id === id ? { ...ride, cancelled: false } : ride,
        ),
      );
